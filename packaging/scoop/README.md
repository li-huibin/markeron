...
- `MarkerOnPlus_1.0.0_x64-setup.exe`
- `MarkerOnPlus_1.0.0_x64-setup.nsis.zip`
- `MarkerOnPlus_1.0.0_x64_zh-CN.msi`
- `MarkerOnPlus_1.0.0_x64.msix`

Recommended next step before submitting to Scoop Extras:

1. Add a true portable Windows zip artifact, for example `MarkerOnPlus_1.0.0_windows_x64_portable.zip`.
2. Ensure it can run from the extracted folder without writing installation metadata.
3. Include `MarkerOnPlus.exe`, required runtime files, icon resources, and an optional shortcut target.
4. Generate SHA256 for the zip and add an autoupdate block.

Draft metadata:

- App name: `markeronplus`
- Description: `Lightweight screen annotation tool with click-through mode`
- Homepage: `https://github.com/ifer47/markeronplus`
- License: `MIT`
- Architecture: `64bit`
- Bin/shortcut target: `MarkerOnPlus.exe`
